...
name: css-animations
description: >-
  中文说明：为HyperFrames编写可定位、可预览且可稳定渲染的CSS动画。
  原始触发说明：CSS animation adapter patterns for HyperFrames. Use when authoring CSS keyframes, animation-delay based timing, animation-fill-mode, animation-play-state, or CSS-only motion that HyperFrames must seek deterministically during preview and rendering.
---
